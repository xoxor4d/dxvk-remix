...
namespace RemixGui {
  ...
  static bool SliderScalar_NoGrabOverlay(const char* label, ImGuiDataType type, void* p_data, const void* p_min, const void* p_max, const char* fmt, ImGuiSliderFlags flags, float overlayAlpha) {
    return withLabeledRow(label, [&]() {
      ...
      ImGui::PushStyleColor(ImGuiCol_SliderGrab, ImVec4(m_dbg_color1[0], m_dbg_color1[1], m_dbg_color1[2], m_dbg_color1[3])/*ImVec4(0, 0, 0, 0)*/);
  ...
  bool TreeNodeBehaviorNoArrow(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end) {
    ...
    const float frame_height = ImMax(ImMin(window->DC.CurrLineSize.y, g.FontSize + style.FramePadding.y * 2), label_size.y + padding.y * 2) + m_dbg_float2;
    ImRect frame_bb;
  ...
  bool CollapsingHeader(const char* label, ImGuiTreeNodeFlags flags/* = 0*/) {
    ...
    if (window->SkipItems) {
      ...
      const float topPad = 8.0f + m_dbg_float1;
      ...
      // Custom uppercase label (left)
      {
        ...
        const ImVec2 textPos(frameBb.Min.x + padding.x, frameBb.Min.y + padding.y + m_dbg_float3);
      ...
      // Custom chevron (right)
      {
        ...
        const ImVec2 arrowPos(frameBb.Max.x - fs - style.FramePadding.x,
                              (frameBb.Min.y + (frameBb.GetHeight() - fs) * 0.5f) + m_dbg_float4);
