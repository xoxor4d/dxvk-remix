...
Wraps [`bridge/scripts/build.ps1`](../../../bridge/scripts/build.ps1),
the fork-side bridge build entry point. The script handles VS discovery
via vswhere, calls vcvarsall.bat by full path (avoiding the upstream
SetupVS lying-on-failure trap), runs `meson setup`/`compile` per arch in
isolated cmd.exe shells (no env contamination between x64 and x86), and
verifies all three expected artifacts.
...
3. Report back:
   ...
   - **Red (non-zero exit):** scan the captured output for
     ...
     first ~10 error lines verbatim. Check whether the failure fits
     the Remix C API multi-layer audit pattern (PFN typedef without
     bare forward decl in `remix_c.h`, impl in anon namespace
     instead of `extern "C"`, missing IPC opcode, missing client
     stub, etc.) before guessing — consult the
     `remix_api_multi_layer_audit` auto-memory entry.
...
> **`d3d9_remix.dll`** in `_output/` is a stale rename from an older
> fork convention. The current build produces `_output/d3d9.dll`. If
> both are present, the older `d3d9_remix.dll` is leftover and safe
> to delete.

> **PDBs may go stale.** The `copy_d3d9_to_output` custom_target
> globs `d3d9*` but in practice the linker may emit the .pdb after
> the copy step has already run, leaving stale debug symbols in
> `_output/`. If you need fresh symbols (crash analysis, dump
> inspection), copy `_compRelease_x86/src/client/d3d9.pdb` to
> `_output/d3d9.pdb` manually after the build.

## Gotchas

- **Don't bypass the script** by calling `build_bridge_release.bat`
  (or `_debug.bat`, `_all.bat`, etc.) directly. Diagnostic session
  2026-05-08 confirmed: under any subprocess context where the parent
  shell hasn't already loaded VS env, the .bat's SetupVS/vcvarsall
  lookup silently fails, the .bat exits 0 with no x86 build output,
  and the .bat's empty captured stdout (`cmd.exe /c` stdio quirk)
  hides the failure. The fork-side `bridge/scripts/build.ps1` was
  written specifically to fix that trap.
